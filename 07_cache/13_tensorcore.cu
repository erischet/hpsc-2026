...
__global__ void kernel(int dim_m, int dim_n, int dim_k,
  ...
  __shared__ half __align__(16) block_a[32][136]; 
  __shared__ half __align__(16) block_b[32][72];
  ...
  for (int k = 0; k < dim_k; k += 32) {
    ...
    for (int k_step = 0; k_step < 2; ++k_step) {
      for (int r = 0; r < 2; r++) {
        ...
        wmma::load_matrix_sync(a_frag[r], &block_a[k_step * 16][row_tile * 16], 136);
      }
      for (int c = 0; c < 4; c++) {
        wmma::load_matrix_sync(b_frag[c], &block_b[k_step * 16][c * 16], 72);
      }
      for (int r = 0; r < 2; r++) {
        for (int c = 0; c < 4; c++) {
